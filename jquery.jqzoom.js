...
{this.node=image[0];this.loadimage=function(){this.node.src=image[0].src;if(typeof smallimagedata.top==="undefined"){this.node.onload();}};this.findborder=function()
...
{a.css({'cursor':'default','display':'inline'});if(a.css('position')!='absolute'&&a.parent().css('position'))
{a.css({'cursor':'default','position':'relative','display':'inline'});}
...
if(settings.lens)
{this.node.style.visibility='hidden';/*document.getElementById("roll_on").innerHTML="Roll off to zoom out";*/}
else
{this.node.style.visibility='hidden';/*document.getElementById("roll_on").innerHTML="Roll on to zoom in";*/$('div.jqZoomPup').hide();}
firstime=1;}else
...
{$('div.jqZoomPup',a).css('display','none');/*document.getElementById("roll_on").innerHTML="Roll on to zoom in";*/var lensleft=(smallimagedata.w)/2-(lensdata.w)/2;var lenstop=(smallimagedata.h)/2-(lensdata.h)/2;this.node.style.left=lensleft+'px';this.node.style.top=lenstop+'px';$('div.jqZoomPup',a).css({top:lenstop,left:lensleft});if(settings.zoomType=='reverse')
{$('div.jqZoomPup img',a).css({'position':'absolute','top':-(lenstop-btop+1),'left':-(lensleft-bleft+1)});}
largeimage.setposition();if($.browser.msie)
{$('div.jqZoomPup',a).show();/*document.getElementById("roll_on").innerHTML="Roll off to zoom out";*/}else
...
{$('div.jqZoomPup',a).remove();/*document.getElementById("roll_on").innerHTML="Roll on to zoom in";*/}};Lens.prototype.findborder=function()
...
{var leftpos=smallimagedata.pos.l;var toppos=smallimagedata.pos.t;this.node=document.createElement("div");$(this.node).addClass('jqZoomWindow');
console.log(jQuery(window).width());
var wid=Math.round(settings.zoomWidth); //width of the zooming window
var hgt=Math.round(settings.zoomHeight);
//varying width according to the width of the screen starts
// if(jQuery(window).width()<1083&&jQuery(window).width()>=1063)  //1083=1100 in chrome 
	// wid=Math.round(settings.zoomWidth)-10;
// if(jQuery(window).width()<1063&&jQuery(window).width()>=1043)
	// wid=Math.round(settings.zoomWidth)-20;
// if(jQuery(window).width()<1043&&jQuery(window).width()>=1023)
	// wid=Math.round(settings.zoomWidth)-30;
// if(jQuery(window).width()<1023&&jQuery(window).width()>983)
	// wid=Math.round(settings.zoomWidth)-40;
// if(jQuery(window).width()<=983&&jQuery(window).width()>=975)
	// wid=Math.round(settings.zoomWidth)-50;
if(jQuery(window).width()<975)
	wid=0; 
//varying width according to the width of the screen ends
$(this.node).css({position:'absolute',width:wid+'px',height:hgt+'px',display:'none',zIndex:100,overflow:'hidden'});switch(settings.position)
...
this.node.style.left=leftpos+5+'px';  //5px away from the image
 imageSize=jQuery('#imageShowcase');     //fixed position of zooming window
                if(imageSize.offset()!=null)
                {
                    imgoff_set=imageSize.offset();
                    img_top=imgoff_set.top; 
                    //jQuery('.jqZoomWindow').css("top",img_top+"px");
					this.node.style.top=img_top+10+'px';
                }/*this.node.style.top=smallimagedata.pos.t+'px';console.log(this.node.style.top);*/return this;}
...
$(this.node).show();if($.browser.msie&&$.browser.version<7){this.ieframe=$('<iframe class="zoom_ieframe" frameborder="0" src="#"></iframe>').css({position:"absolute",left:this.node.style.left,top:smallimagedata.pos.t+'px',zIndex:100,width:settings.zoomWidth,height:settings.zoomHeight}).insertBefore(this.node);};largeimage.node.style.display='block';}
